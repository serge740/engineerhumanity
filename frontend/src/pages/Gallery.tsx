...
const rwandaModules = import.meta.glob(
  '../assets/events/rwandaconvention July 2025 (1)/*.{jpg,JPG,jpeg,png,PNG}',
  { eager: true, query: '?url', import: 'default' }
) as Record<string, string>;

const refugeeDay2025Modules = import.meta.glob(
  '../assets/events/International World Refugee Day 2025, June 21/*.{png,jpg,jpeg,JPG,PNG}',
  { eager: true, query: '?url', import: 'default' }
) as Record<string, string>;

const refugeeDay2024Modules = import.meta.glob(
  '../assets/events/Event International Refugee day/*.{png,jpg,jpeg,JPG,PNG}',
  { eager: true, query: '?url', import: 'default' }
) as Record<string, string>;

const refugeeDayRecognitionModules = import.meta.glob(
  '../assets/events/World Refugee Day Recognition/*.{png,jpg,jpeg,JPG,PNG}',
  { eager: true, query: '?url', import: 'default' }
) as Record<string, string>;

const upcomingModules = import.meta.glob(
  '../assets/events/events upcoming/*.{png,jpg,jpeg,JPG,PNG}',
  { eager: true, query: '?url', import: 'default' }
) as Record<string, string>;

const firstIntakeGalleryModules = import.meta.glob(
  '../assets/events/first intake/*.{jpeg,jpg,JPG,png,PNG}',
  { eager: true, query: '?url', import: 'default' }
) as Record<string, string>;

const vocTrainingGalleryModules = import.meta.glob(
...
function buildImages(): GalleryImage[] {
  ...
  for (const src of Object.values(refugeeDay2025Modules).sort()) {
    result.push({ id: id++, src, title: "World Refugee Day 2025 – Dallas Celebration", category: "World Refugee Day", location: "Richardson, Texas, USA", date: "June 21, 2025" });
  }
  for (const src of Object.values(refugeeDay2024Modules).sort()) {
    result.push({ id: id++, src, title: "World Refugee Day 2024 – Dallas Celebration", category: "World Refugee Day", location: "Dallas, Texas, USA", date: "June 22, 2024" });
  }
  for (const src of Object.values(refugeeDayRecognitionModules).sort()) {
    result.push({ id: id++, src, title: "Dallas County – World Refugee Day Recognition", category: "World Refugee Day", location: "Dallas County, Texas, USA", date: "June 17, 2025" });
  }
  for (const src of Object.values(upcomingModules).sort()) {
    result.push({ id: id++, src, title: "Upcoming Events", category: "Upcoming Events", location: "USA & Rwanda", date: "2025–2027" });
  }
  for (const src of Object.values(firstIntakeGalleryModules).sort()) {
    result.push({ id: id++, src, title: "Hope School First Intake Graduation", category: "Education", location: "Gihembe Refugee Camp, Rwanda", date: "March 2012" });
  }
  for (const src of Object.values(vocTrainingGalleryModules).sort()) {
    result.push({ id: id++, src, title: "Vocational Training & Research Center Launch", category: "Training", location: "Gihembe Refugee Camp, Rwanda", date: "2020" });
  }
