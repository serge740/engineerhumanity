...
function HighlightIcon({ icon }: { icon: string }) {
  ...
  // default: chart (also covers 'heart', matching the original site's behavior)
  return (
    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function PastEventDetailPublicPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <EventHero title={event.title} linkTitle="Past Events" linkHref="/past-event" backgroundImage={coverImage} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mb-14">
          <div className="lg:col-span-2 space-y-5">
            ...
            {event.paragraphs.map((para, i) => (
              <p key={i} className="text-gray-700 leading-relaxed text-base" dangerouslySetInnerHTML={{ __html: para }} />
            ))}
          </div>
          ...
          <div className="space-y-4">
            ...
            {images.length > 0 && (
              <div className="mt-4 bg-green-50 rounded-xl p-4 border border-green-100 flex items-center gap-3">
                ...
                <span className="font-semibold text-gray-800 text-sm">{images.length} Event Photos</span>
              </div>
            )}
          </div>
        </div>
        ...
        <section className="bg-gray-50 py-14">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              ...
              <p className="text-gray-500">{images.length} photos from this event — click to view full size</p>
            </div>
      ...
      <section className="bg-gradient-to-r from-blue-600 to-sky-600 text-white py-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/donate" className="bg-white text-blue-600 px-8 py-4 rounded-lg font-bold hover:bg-gray-100 transition-colors duration-300 shadow-lg">
              Donate Now
            </Link>
            <Link to="/past-event" className="border-2 border-white text-white px-8 py-4 rounded-lg font-bold hover:bg-white/10 transition-colors duration-300">
              ← All Past Events
            </Link>
          </div>
        </div>
      ...
      {lightboxIndex !== null && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4" onClick={closeLightbox}>
          <button className="absolute top-4 right-4 bg-white/20 hover:bg-white/30 text-white rounded-full p-3 transition-colors" onClick={closeLightbox}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>

          <button className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white rounded-full p-3 transition-colors" onClick={(e) => { e.stopPropagation(); prevImage(); }}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          ...
          <button className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white rounded-full p-3 transition-colors" onClick={(e) => { e.stopPropagation(); nextImage(); }}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
